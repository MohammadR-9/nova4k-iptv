...
export const ActivationLogin: React.FC<ActivationLoginProps> = ({ 
  ...
  const fillDemoCredentials = () => {
    setUsername('vip_user');
    setPassword('pass7788');
    setServerUrl(SERVER_CONFIG.DEFAULT_PORTAL_URL);
    setActiveTab('credentials');
    setErrorMsg(null);
    setTimeout(() => {
      spatialNav.setFocus('btn-submit-creds');
    }, 100);
  };
  ...
  const fillDemoCode = () => {
    ...
    setErrorMsg(null);
    setTimeout(() => {
      spatialNav.setFocus('btn-submit-code');
    }, 100);
  };
  ...
  return (
    <div className="relative w-full max-w-full h-full min-h-[100dvh] flex items-center justify-center bg-radial-vignette overflow-x-hidden overflow-y-auto px-4 sm:px-6 py-6 sm:py-8 select-none touch-pan-y overscroll-x-none">
      ...
      {/* Main Login Card */}
      <div className="relative w-full max-w-[95vw] sm:max-w-xl lg:max-w-5xl xl:max-w-6xl bg-surface-primary/95 border border-white/10 rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 lg:p-10 shadow-2xl backdrop-blur-2xl flex flex-col my-auto">
        ...
        {onOpenAdminPortal && (
          <button
            ...
            className="absolute top-4 left-4 p-2 rounded-xl bg-white/5 hover:bg-cyan-500/20 text-slate-400 hover:text-cyan-300 border border-white/10 transition-all cursor-pointer flex items-center gap-1 text-[11px] z-10"
        ...
        <div className="w-full lg:grid lg:grid-cols-12 lg:gap-8 items-start">
          {/* Side Column (Right on RTL): Branding + Live Server + Quick 1-Click Demo + TV Remote Guide */}
          <div className="lg:col-span-5 flex flex-col items-center lg:items-end text-center lg:text-right gap-4 mb-6 lg:mb-0">
            {/* App Logo & Secret Developer Portal Trigger */}
            <div 
              onClick={handleLogoClick}
              className="flex flex-col items-center lg:items-end cursor-pointer group transition-transform active:scale-95"
            ...
            {/* Live Server Connectivity Status Bar */}
            <div className="w-full flex items-center justify-between px-4 py-2.5 bg-surface-elevated/80 border border-white/5 rounded-2xl">
              <div className="flex items-center gap-2.5">
                ...
                <div className="text-right">
                  <span className="text-slate-400 text-[10px] block leading-tight">سيرفر NOVA 4K الرسمي:</span>
            ...
            <div className="w-full bg-gradient-to-b from-cyan-950/30 to-purple-950/20 border border-nova-cyan/20 rounded-2xl p-3.5 flex flex-col gap-2.5 text-right">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono px-2 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/30">
                  DEMO PASS
                </span>
                <span className="text-xs font-black text-white flex items-center gap-1.5">
                  <Zap className="w-3.5 h-3.5 text-nova-gold fill-nova-gold" />
                  <span>دخول تجريبي فوري بنقرة واحدة</span>
                </span>
              </div>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                لا داعي للكتابة عبر الريموت! اضغط على أي زر لتعبئة بيانات التجربة فوراً:
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-1 gap-2">
                <button
                  data-nav-id="btn-demo-creds-side"
                  type="button"
                  onClick={fillDemoCredentials}
                  className="tv-focusable py-2 px-3 rounded-xl bg-white/5 hover:bg-cyan-500/20 border border-white/10 hover:border-cyan-400 text-slate-200 hover:text-white text-xs font-bold flex items-center justify-between transition-all cursor-pointer"
                >
                  <span className="font-mono text-[11px] text-cyan-300">vip_user / pass7788</span>
                ...
                <button
                  data-nav-id="btn-demo-code-side"
                  type="button"
                  onClick={fillDemoCode}
                  className="tv-focusable py-2 px-3 rounded-xl bg-white/5 hover:bg-purple-500/20 border border-white/10 hover:border-purple-400 text-slate-200 hover:text-white text-xs font-bold flex items-center justify-between transition-all cursor-pointer"
                >
                  <span className="font-mono text-[11px] text-purple-300">882419</span>
                  <span className="flex items-center gap-1">
                    <KeyRound className="w-3.5 h-3.5 text-nova-purple" />
                    <span>كود تفعيل</span>
                  </span>
                </button>
              </div>
            ...
            {/* TV Remote Tips Guide */}
            <div className="hidden lg:flex w-full bg-white/5 border border-white/10 rounded-2xl p-3 flex-col gap-1.5 text-[11px] text-slate-400 text-right">
              <div className="flex items-center gap-1.5 font-bold text-white mb-0.5">
                <Gamepad2 className="w-4 h-4 text-nova-cyan" />
                <span>إرشادات ريموت التلفاز:</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-300">التنقل السلس بين الخانات</span>
                <span className="font-mono text-[10px] text-cyan-400 bg-black/40 px-1.5 py-0.5 rounded">الأسهم ▲ ▼ ◄ ►</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-300">فتح الكيبورد الداخلي</span>
                <span className="font-mono text-[10px] text-cyan-400 bg-black/40 px-1.5 py-0.5 rounded">زر OK / Enter</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-300">الخروج الآمن من التطبيق</span>
                <span className="font-mono text-[10px] text-amber-400 bg-black/40 px-1.5 py-0.5 rounded">زر Back / Esc</span>
              </div>
            </div>
          </div>

          {/* Main / Left Column: Tabs, Forms, Actions */}
          <div className="lg:col-span-7 flex flex-col w-full">
            {/* Auth Mode Tabs (Profiles vs Code vs Credentials) */}
            <div className="w-full grid grid-cols-3 gap-1.5 p-1 bg-surface-elevated rounded-2xl mb-4 border border-white/5">
              <button
                data-nav-id="btn-tab-profiles"
                data-nav-group="auth-tabs"
                data-nav-right="btn-tab-code"
                data-nav-down={profiles.length > 0 ? "btn-profile-login-0" : "btn-quick-fill-creds"}
              ...
              <button
                ...
                data-nav-group="auth-tabs"
                data-nav-left="btn-tab-profiles"
                data-nav-right="btn-tab-creds"
                data-nav-down="btn-quick-fill-code"
                onClick={() => { setActiveTab('code'); spatialNav.setFocus('btn-quick-fill-code'); setErrorMsg(null); }}
                className={`tv-focusable flex items-center justify-center gap-1.5 py-2.5 sm:py-3 rounded-xl font-bold text-xs sm:text-sm md:text-base transition-all ${
                  activeTab === 'code' 
              ...
              <button
                ...
                data-nav-group="auth-tabs"
                data-nav-left="btn-tab-code"
                data-nav-down="btn-quick-fill-creds"
                onClick={() => { setActiveTab('credentials'); spatialNav.setFocus('btn-quick-fill-creds'); setErrorMsg(null); }}
                className={`tv-focusable flex items-center justify-center gap-1.5 py-2.5 sm:py-3 rounded-xl font-bold text-xs sm:text-sm md:text-base transition-all ${
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
                        ...
                        <button
                          ...
                          className="tv-focusable px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 font-black text-xs sm:text-sm flex items-center gap-1.5 shadow-md shadow-cyan-500/20 active:scale-95 transition-all cursor-pointer disabled:opacity-50"
                        >
            ...
            {activeTab === 'code' && (
              <div className="w-full flex flex-col items-center gap-3.5">
                {/* 1-Click Code Demo Fill Button */}
                <button
                  data-nav-id="btn-quick-fill-code"
                  data-nav-up="btn-tab-code"
                  data-nav-down="input-code"
                  type="button"
                  onClick={fillDemoCode}
                  className="tv-focusable w-full py-2.5 px-4 rounded-2xl bg-gradient-to-r from-purple-500/15 via-cyan-500/15 to-purple-500/15 hover:from-purple-500/25 hover:to-cyan-500/25 border border-purple-400/40 text-purple-200 hover:text-white text-xs font-bold flex items-center justify-between transition-all cursor-pointer shadow-sm"
                >
                  <span className="flex items-center gap-2">
                    <Zap className="w-4 h-4 text-nova-gold fill-nova-gold" />
                    <span>تعبئة كود التجربة بنقرة واحدة</span>
                  </span>
                  <span className="font-mono text-xs bg-black/50 px-2.5 py-0.5 rounded-lg text-cyan-300 font-bold">
                    882419
                  </span>
                </button>
                ...
                <div className="w-full">
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        data-nav-up="btn-quick-fill-code"
                        data-nav-down="btn-submit-code"
                        data-nav-left="btn-vk-code"
                        type="text"
                        value={code}
                        ...
                        className="tv-focusable w-full h-14 md:h-16 bg-surface-elevated/90 border-2 border-white/10 rounded-2xl px-6 text-center text-2xl md:text-3xl font-mono tracking-widest text-accent-cyan placeholder:text-slate-600 focus:outline-none focus:border-accent-cyan cursor-pointer"
                      />
                      <KeyRound className="absolute right-4 top-4 md:top-5 w-5 h-5 text-slate-500 pointer-events-none" />
                    </div>
                    <button
                      data-nav-id="btn-vk-code"
                      data-nav-up="btn-quick-fill-code"
                      data-nav-down="btn-submit-code"
                      data-nav-right="input-code"
                      type="button"
                      onClick={() => openVirtualKeyboard('code', 'كود التفعيل (Activation Code)', 'مثال: 882419', code, false)}
                  ...
                  <p className="text-[11px] text-slate-500 mt-2 text-right leading-relaxed">
                    * اضغط OK على الخانة لفتح لوحة المفاتيح الداخلية للشاشة، أو استخدم زر التعبئة السريع أعلاه.
                  </p>
                </div>
                      ...
                      <div className="mt-2 p-3 rounded-2xl bg-white/5 border border-white/10">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            className="tv-focusable flex-1 h-11 bg-black/50 border border-white/10 rounded-xl px-4 text-xs font-mono text-white placeholder:text-slate-500 focus:outline-none focus:border-accent-cyan cursor-pointer"
                          />
                          <button
            ...
            {activeTab === 'credentials' && (
              <div className="w-full flex flex-col gap-3.5">
                {/* 1-Click Credentials Demo Fill Button */}
                <button
                  data-nav-id="btn-quick-fill-creds"
                  data-nav-up="btn-tab-creds"
                  data-nav-down="input-user"
                  type="button"
                  onClick={fillDemoCredentials}
                  className="tv-focusable w-full py-2.5 px-4 rounded-2xl bg-gradient-to-r from-amber-500/15 via-cyan-500/15 to-purple-500/15 hover:from-amber-500/25 hover:to-cyan-500/25 border border-amber-400/40 text-amber-300 hover:text-amber-200 text-xs font-bold flex items-center justify-between transition-all cursor-pointer shadow-sm"
                >
                  <span className="flex items-center gap-2">
                    <Zap className="w-4 h-4 text-amber-400 fill-amber-400" />
                    <span>تعبئة بيانات التجربة بنقرة واحدة (VIP Demo)</span>
                  </span>
                  <span className="font-mono text-xs bg-black/50 px-2.5 py-0.5 rounded-lg text-cyan-300 font-bold">
                    vip_user / pass7788
                  </span>
                  ...
                  <div className="flex items-center gap-2">
                    <div className="relative flex-1">
                      <input
                        ...
                        data-nav-up="btn-quick-fill-creds"
                        data-nav-down="input-pass"
                        data-nav-left="btn-vk-user"
                        type="text"
                        value={username}
                      ...
                      />
                      <User className="absolute left-4 top-3.5 w-4 h-4 md:w-5 md:h-5 text-slate-500 pointer-events-none" />
                    </div>
                    <button
                      data-nav-id="btn-vk-user"
                      data-nav-up="btn-quick-fill-creds"
                      data-nav-down="btn-vk-pass"
                      data-nav-right="input-user"
                      type="button"
                      onClick={() => openVirtualKeyboard('username', 'اسم المستخدم (Username)', 'Username', username, false)}
                      ...
                      <div className="mt-2 p-3 rounded-2xl bg-white/5 border border-white/10">
                        ...
                        <div className="flex items-center gap-2">
                          <input
                            ...
                            placeholder="http://my-iptv-server.com:8080"
                            className="tv-focusable flex-1 h-11 bg-black/50 border border-white/10 rounded-xl px-4 text-xs font-mono text-white placeholder:text-slate-500 focus:outline-none focus:border-accent-cyan text-left cursor-pointer"
                          />
                          <button
