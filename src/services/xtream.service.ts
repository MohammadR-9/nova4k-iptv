...
export class XtreamService {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(remoteStreams) && remoteStreams.length > 0) {
            const mapped: LiveChannel[] = remoteStreams.map((s: any, idx: number) => ({
              ...
              // Xtream Codes standard .m3u8 HLS stream for cross-platform compatibility
  ...
  public static getCuratedLiveChannels(categoryId: string = 'all'): LiveChannel[] {
    const curatedChannels: LiveChannel[] = [
      ...
      {
        num: 2,
        name: 'beIN SPORTS 2 UHD (HEVC HDR)',
        stream_type: 'live',
        stream_id: 102,
        ...
        custom_sid: null,
        tv_archive: 1,
        direct_source: 'https://devstreaming-cdn.apple.com/videos/streaming/examples/bipbop_adv_example_hevc/master.m3u8',
        resolution: '4K UHD',
        fps: 60,
        isFavorite: true,
        currentProgram: {
